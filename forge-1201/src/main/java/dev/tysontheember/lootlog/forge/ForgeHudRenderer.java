...
/**
 * Forge-specific HUD renderer. Registered as a custom GUI overlay
 * and implements RenderBridge by wrapping GuiGraphics.
 */
public class ForgeHudRenderer implements RenderBridge {
    ...
    /** Called from RegisterGuiOverlaysEvent to register the overlay. */
    public static void registerOverlay(RegisterGuiOverlaysEvent event) {
        event.registerAboveAll(LootLog.MOD_ID, ForgeHudRenderer::renderOverlay);
    }
    ...
    private static void renderOverlay(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ForgeHudRenderer renderer = (ForgeHudRenderer) LootLog.getRenderBridge();
        if (renderer == null) return;

        renderer.graphics = guiGraphics;
        LootLog.getTracker().tick();
        LootLog.getHudLayout().render(LootLog.getTracker().getEntries(), LootLog.getConfig(), renderer);
